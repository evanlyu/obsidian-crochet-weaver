...
| `style` | `radial` \| `japanese` \| `continuous` | plugin setting | Round-chart drawing style. `japanese` gives Japanese-pattern-book styling: a continuous spiral guide winds through the rounds, ordinary stitches follow recorded parents, increases open as balanced **V** marks, decreases are **∧** marks, rounds are numbered, and an `in MR` center is printed as `わ`. An `in ch ring` center keeps its chain-stitch ring rather than using `ち`. `continuous` uses the same ancestry layout but draws every stitch's own symbol and links it to the stitch below. With automatic spacing a crowded round may grow. An explicit `spacing` stays exact and identical between every pair of rounds; the whole chart may raise round 1's radius so configured-size symbols fit, while collisions trigger a deterministic minimum-movement clearance pass without local per-round scaling. Corrected bearings reconcile inward only when ancestry covers every preceding-round source exactly once; partial or repeated source use stops that correction so earlier stitches and V marks keep their shape. Deliberate skips never trigger unrelated even spacing. The number/separator packet moves only inside measured seam surplus, follows a continuously clear path, and stops at the first collision so it remains on the opening side of its separator and wholly before its closing join; partial charts use the same rule. Only affects `type: round`. |
...
For a graph-driven round chart with explicit `spacing`, the gap between every pair of rounds remains exact. If a later round suddenly contains far more or wider symbols, the chart measures all rounds first and raises the absolute radius of round 1 only as much as required; it never overlaps, hides, or locally shrinks stitches to keep an impossible smaller circumference. Projected bearings reconcile inward only through ancestry that covers every preceding-round source exactly once. Deliberate partial placement or repeated reuse of a source stops that correction, preserving earlier stitches and balanced V marks.
