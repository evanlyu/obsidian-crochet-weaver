...
| `style` | `radial` \| `japanese` \| `continuous` | plugin setting | Round-chart drawing style. `japanese` gives Japanese-pattern-book styling: a continuous spiral guide winds through the rounds, ordinary stitches follow recorded parents, increases open as balanced **V** marks, decreases are **∧** marks, rounds are numbered, and an `in MR` center is printed as `わ`. An `in ch ring` center keeps its chain-stitch ring rather than using `ち`. `continuous` uses the same ancestry layout but draws every stitch's own symbol and links it to the stitch below. With automatic spacing a crowded round may grow. An explicit `spacing` stays exact and identical between every pair of rounds; exact parent angles are retained while configured-size symbols fit, while inherited collisions trigger a deterministic minimum-movement clearance pass without local per-round scaling. Corrected bearings reconcile inward through recorded ancestry, keeping later stitches on their displayed parent or V endpoint and every V centred on its real source. Every round therefore retains the symbol size resolved from the user's setting. This prevents overlaps and invisible stitches without rules tied to a particular round or count. Deliberate skips never trigger unrelated even spacing, and the number/separator packet moves only inside measured seam surplus. Every round number follows the continuously clear path from its measured safe slot and stops at the first collision, so it remains on the opening side of its separator and wholly before its closing join; partial charts that begin at any round use the same rule. Only affects `type: round`. |
