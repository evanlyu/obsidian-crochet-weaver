...
| `style` | `radial` \| `japanese` \| `continuous` | 外掛設定值 | 環織織圖的繪製樣式。`japanese` 為日本鉤織書風格：一條連續螺旋線繞過每一圈、普通針跟隨記錄的父針、加針以父針為中線畫成平衡 **V**、減針畫成 **∧**、標示圈數，且把 `in MR` 中心印成 `わ`。`in ch ring` 中心仍畫成鎖針符號環，不使用 `ち`。`continuous` 使用同一套來源排版，但每一針都畫出自己的符號並連線到下方針目。使用自動圈距時，擁擠的圈可以擴大；明確指定 `spacing` 時，每兩圈都嚴格使用同一個精確距離。只要設定尺寸的針目能排開就保留父針角度；繼承位置若發生碰撞，系統會用確定性的最小位移恢復淨空，不再套用每圈各自的局部縮放。修正角度會沿記錄的來源關係往內協調，讓外圈針目仍對準畫面上的父針或 V 端點，且每個 V 仍以真實來源為中線。所有圈都會保留使用者設定所解析出的針目尺寸。這能防止重疊與隱形針目，也不依賴特定圈數或針數。刻意跳針不會觸發無關的平均分佈，圈號／分隔線封包也只能在量得的縫隙餘裕內移動。每個圈號會從量得的安全位置沿連續無碰撞路徑前進，並在第一個碰撞處停下，所以會留在分隔線的起圈側並完整位於白色接合點左側；從任何圈數開始的截取織圖也使用同一規則。只影響 `type: round`。 |
