...
| `id` | 1–80 個 ASCII 英文字母、數字、`_` 或 `-` | 由區塊內容推導 | 當你同時內嵌或撰寫 `crochet-tool`／`tool: on` 面板時，務必設定這個值，這樣即使之後編輯織圖文字，進度也不會遺失。 |
...
| `style` | `radial` \| `japanese` \| `continuous` | 外掛設定值 | 環織織圖的繪製樣式。`japanese` 為日本鉤織書風格：一條連續螺旋線繞過每一圈、普通針跟隨記錄的父針、加針以父針為中線畫成平衡 **V**、減針畫成 **∧**、標示圈數，且把 `in MR` 中心印成 `わ`。`in ch ring` 中心仍畫成鎖針符號環，不使用 `ち`。`continuous` 使用同一套來源排版，但每一針都畫出自己的符號並連線到下方針目。使用自動圈距時，擁擠的圈可以擴大；明確指定 `spacing` 時，每兩圈都嚴格使用同一個精確距離。整張圖可以提高第 1 圈半徑，讓設定尺寸的針目排開；繼承位置若發生碰撞，系統會用確定性的最小位移恢復淨空，不套用每圈各自的局部縮放。修正角度只有在來源關係恰好覆蓋前一圈每個來源一次時才往內協調；部分或重複來源會停止回寫，讓較早圈的針目與 V 保持原形。刻意跳針不會觸發無關的平均分佈。圈號／分隔線封包只能在量得的縫隙餘裕內沿連續無碰撞路徑移動，並在第一個碰撞處停下，因此會留在分隔線的起圈側並完整位於白色接合點左側；截取織圖也使用同一規則。只影響 `type: round`。 |
| `lace` | 布林值（同上） | `off` | 鉤織書式的蕾絲顯示：隱藏圈線與圈號、放大符號，並讓鎖針串向開放空間展開。 |
| `sector` | `on` 或 0 到 360 之間的數字 | 完整織圖 | 只畫環織織圖的一個扇形；`on` 代表 90 度。 |
| `wholeRounds` | 正整數 | 無 | 套用 `sector` 前，保留這麼多個中心圈為完整圓。 |
| `grid` | 布林值（同上） | 外掛設定值 | 畫出對齊真實織圖的背景參考網格。 |
| `rounds` / `rows` / `columns` | 正整數 | 織圖真實範圍 | 讓背景參考網格延伸超出真實織圖，但不會縮小。 |
| `tool` | 布林值（同上） | 外掛設定值 | 在織圖旁內嵌互動式進度工具。見「內嵌進度面板」。 |
| `text` | 布林值（同上） | 外掛設定值 | 在織圖旁內嵌唯讀的縮寫清單（若 `tool` 也開啟則會被忽略）。 |
| `readable` | 布林值（同上） | 外掛設定值 | 在工具／文字面板顯示完整翻譯的針法名稱，而不是原始簡碼。 |
| `position` | `right` \| `left` \| `below` | 外掛設定值 | 內嵌的 `tool`／`text` 面板相對於織圖要放在哪個位置。 |

無效值會安靜地退回對應的全域設定；若該選項沒有全域預設（例如 `sector`），則直接忽略──不會報錯，所以對不確定的值要自行再三確認，不要指望會有明顯的失敗提示。

圖關係驅動的環織織圖若明確設定 `spacing`，每兩圈之間都會維持精確間距。若後面某圈突然出現更多或更寬的符號，系統會先量完整張圖，只把第 1 圈的絕對半徑提高到剛好足以容納；不會為了維持不可能的小圓周而讓針目重疊、消失或局部縮小。投影後的角度只有在來源關係「恰好完整覆蓋前一圈每個來源一次」時才會往內協調；刻意只選部分位置或重複使用同一來源時會停止回寫，保留較早圈的針目位置與平衡 V 形。
...
每圈開頭的起立鎖針**是**支援的：請保留原文的普通或加註 `ch` 步驟（`R2: ch, [2 sc, inc] rep, slst`），它會畫在接縫處。`ch 3 (counts as dc)` 算 1 針替代針，`ch 1 (does not count as a st)` 算 0；沒有加註的起始鎖針，只有在該圈最後接合到它的頂端時才算 1。結尾接合的 `sl st` 與寫成步驟的 `mr`（`R1: mr, ch, sc6, slst`）一律算 0。
...
它本身不佔寬度（不算一針）。從這個步驟開始，之後的每一針──包含這一行剩下的部分，以及之後所有行──都套用這個顏色，並照常計數，直到下一個 `color` 步驟改變顏色為止；沒有「恢復成無顏色」的寫法。針目符號本身一律維持織圖原本的主題色──`black`／`white` 這類字面顏色只是用來標記毛線顏色，不會真的畫成那個顏色（在深色或淺色主題下都可能變得看不清楚，也會跟進度工具自己的「目前位置」標示互相干擾）。取而代之的是：每次換成新顏色的第一針，會在旁邊畫一個該顏色的空心圓圈，讓換色位置一眼就能看出來，同時又不會遮住底下的針目符號。進度工具與織圖文字面板也會直接寫出「換成 `<color>`」提示換線的位置。
...
一圈提供給下一圈鉤入的「位置」，包含它的針目（`st`、`sc`、`dc`、`picot`…）**以及針目之間的鎖針空間**：任何寫在兩針之間的連續鎖針，都會形成一個 `ch-N sp`。`next <place>` 會自動略過中間的東西，所以你不需要把跳過的針寫出來──原文也沒有寫。
...
- `tool: on` ── 在織圖旁顯示完整互動檢查清單：逐行列表、進度條，以及依權重前進的每行針數計數器。每完成一個書寫單位按一次；按鈕會增加該單位的權重（`inc` 是 `+2`，V 針／貝殼針會一次增加完整權重）。織圖也會即時標示目前的行／針。
...
10. 在原文明確換線的地方加上 `color <name>` 步驟，使用原文寫的確切顏色字或色碼；原文沒提到顏色的地方就不要加。
