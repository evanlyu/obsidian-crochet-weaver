...
| `id` | 1–80 个 ASCII 英文字母、数字、`_` 或 `-` | 由代码块内容推导 | 当你同时内嵌或编写 `crochet-tool`／`tool: on` 面板时，务必设置这个值，这样即使之后编辑织图文本，进度也不会丢失。 |
...
| `style` | `radial` \| `japanese` \| `continuous` | 插件设置值 | 环织织图的绘制样式。`japanese` 为日本钩织书风格：一条连续螺旋线绕过每一圈、普通针跟随记录的父针、加针以父针为中线画成平衡 **V**、减针画成 **∧**、标注圈数，并把 `in MR` 中心印成 `わ`。`in ch ring` 中心仍绘制锁针符号环，不使用 `ち`。`continuous` 使用同一套来源排版，但每一针都画出自己的符号并连线到下方针目。使用自动圈距时，拥挤的圈可以扩大；明确指定 `spacing` 时，每两圈都严格使用同一个精确距离。整张图可以提高第 1 圈半径，让设置尺寸的针目排开；继承位置如果发生碰撞，系统会用确定性的最小位移恢复净空，不套用每圈各自的局部缩放。修正角度只有在来源关系恰好覆盖前一圈每个来源一次时才向内协调；部分或重复来源会停止回写，让较早圈的针目与 V 保持原形。刻意跳针不会触发无关的平均分布。圈号／分隔线组合只能在测得的接缝余量内沿连续无碰撞路径移动，并在第一个碰撞处停下，因此会留在分隔线的起圈侧并完整位于白色接合点左侧；截取织图也使用同一规则。只影响 `type: round`。 |
| `lace` | 布尔值（同上） | `off` | 钩织书式的蕾丝显示：隐藏圈线与圈号、放大符号，并让锁针串向开放空间展开。 |
| `sector` | `on` 或 0 到 360 之间的数字 | 完整织图 | 只画环织织图的一个扇形；`on` 表示 90 度。 |
| `wholeRounds` | 正整数 | 无 | 应用 `sector` 前，保留这么多个中心圈为完整圆。 |
| `grid` | 布尔值（同上） | 插件设置值 | 绘制与真实织图对齐的背景参考网格。 |
| `rounds` / `rows` / `columns` | 正整数 | 织图真实范围 | 让背景参考网格延伸超出真实织图，但不会缩小。 |
| `tool` | 布尔值（同上） | 插件设置值 | 在织图旁内嵌交互式进度工具。见“内嵌进度面板”。 |
| `text` | 布尔值（同上） | 插件设置值 | 在织图旁内嵌只读的缩写列表（如果 `tool` 也开启则会被忽略）。 |
| `readable` | 布尔值（同上） | 插件设置值 | 在工具／文字面板显示完整翻译的针法名称，而不是原始简码。 |
| `position` | `right` \| `left` \| `below` | 插件设置值 | 内嵌的 `tool`／`text` 面板相对于织图放在哪个位置。 |

无效值会静默地回退到对应的全局设置；如果该选项没有全局默认值（例如 `sector`），则直接忽略——不会报错，所以对不确定的值要自己反复确认，不要指望会有明显的失败提示。

由图关系驱动的环织织图如果明确设置 `spacing`，每两圈之间都会保持精确间距。如果后面某一圈突然出现更多或更宽的符号，系统会先测量整张图，只把第 1 圈的绝对半径提高到刚好足够容纳；不会为了维持不可能的小圆周而让针目重叠、消失或局部缩小。投影后的角度只有在来源关系“恰好完整覆盖前一圈每个来源一次”时才会向内协调；刻意只选择部分位置或重复使用同一来源时会停止回写，保留较早圈的针目位置与平衡 V 形。
...
每圈开头的起立锁针**是**支持的：请保留原文的普通或加注 `ch` 步骤（`R2: ch, [2 sc, inc] rep, slst`），它会画在接缝处。`ch 3 (counts as dc)` 算 1 针替代针，`ch 1 (does not count as a st)` 算 0；没有加注的起始锁针，只有在该圈最后接合到它的顶端时才算 1。结尾接合的 `sl st` 与写成步骤的 `mr`（`R1: mr, ch, sc6, slst`）始终算 0。
...
它本身不占宽度（不算一针）。从这个步骤开始，之后的每一针——包括这一行剩下的部分，以及之后所有行——都套用这个颜色，并照常计数，直到下一个 `color` 步骤改变颜色为止；没有“恢复成无颜色”的写法。针目符号本身始终保持织图原本的主题色——`black`／`white` 这类字面颜色只是用来标记毛线颜色，并不会真的画成那个颜色（在深色或浅色主题下都可能变得难以辨认，也会和进度工具自己的“当前位置”标记互相干扰）。取而代之的是：每次换成新颜色的第一针旁边会画一个该颜色的空心圆圈，让换色位置一眼就能看出来，同时不会遮住底下的针目符号。进度工具与织图文字面板也会直接写出“换成 `<color>`”来提示换线的位置。

请直接使用原文写的颜色词或色码（`white`、`black`、`#ff8800` 等）——不要自行发明或转换原文没提到的颜色，原文完全没提到颜色的部件也不要加上 `color` 步骤。

### 蕾丝：说明每一针钩在哪裡

蕾丝织图不是「数着」前一圈钩過去，而是直接说明每一针钩在哪裡。请照原文的写法，用 `in` 加上位置：

```
sc in next dc                          → 钩入下一针長针
5 dc in next ch-2 sp                   → 五针長针，全部钩入下一个锁针空间
dc in same st                          → 钩入上一步剛用過的同一个位置
sc in center dc of next 7-dc shell     → 下一组 7 長针贝壳的正中央那一针
sl st into next ch-1 sp                → 引拔移動到該空间；此时还没有钩入任何针
skip 2                                 → 刻意跳過兩个位置
```

一圈提供給下一圈钩入的「位置」，包含它的针目（`st`、`sc`、`dc`、`picot`…）**以及针目之间的锁针空间**：任何寫在兩针之间的連續锁针，都會形成一个 `ch-N sp`。`next <place>` 會自動略過中间的東西，所以你不需要把跳過的针寫出來──原文也没有写。

**贝壳與 V 针。** 「数量 + 位置」是一个钩在同一个位置的花樣，会画成从该位置展开的扇形：

```
5 dc in next ch-2 sp                   → 一组五長针贝壳
V2 in next sc                          → (dc, ch 2, dc) 钩入同一针；V3 是 ch-3 的版本
(dc, ch 2, dc) in next sc              → 同样的东西，写成完整形式
```

V 针內部的锁针本身就是一个空间，所以下一圈可以写 `in next ch-2 sp`。奇數针數的贝壳有中央针，后面的圈数可以指名：`in center dc of next 5-dc shell`。

**起始锁针、接合、翻面。**

```
ch 3 (counts as dc)                    → 這串锁针代替該圈的第一针長针
ch 1 (does not count as a st)          → 只是立起高度
sl st to top of beginning ch-3         → 引拔接合到起始锁针頂端
sl st to first sc
sl st to join
turn                                   → 写在最前面：這一圈要翻面往反方向钩
```

**没有**加註的起始锁针，由该圈自己的接合决定：接合到起始锁针頂端，就代表它代替一针；接合到别处，就代表不算针數。所以原文只写 `ch 3` 并接合 `top of beginning ch-3` 时，不需要帮它补上注记──照原文写即可。

**写成「重复前面某圈」的圈数。** 两种破折号都可以，每一圈都会在画图前展开成真正的一圈：
...
**照书上的样子印。** 在蕾丝织图的 frontmatter 加上 `lace: on`：不画每一圈的框线、不标圈数，针目也画得更大以衬托镂空。它只改变织图周围画了什么，完全不影响织图本身——用户想要钩织书的样子时就加上，想要框出并标示圈数时就不要加。

**只画一块扇形，而不是整圈。** `sector: 90`（或 `sector: on`）搭配 `wholeRounds: 4`（前几圈完整画出）只画圈织图的一块切片——钩织书遇到十二个相同花样的圈就是这样印的。用户希望整张图看起来是扇形而不是完整圆盘时、或整圈太大看不清楚时就加上；想看到每一个重复时就不要加。

**一圈可以写成好几行。** 有缩进的行属于上面那一圈，空行代表该圈结束——所以长的一圈可以保持原文印出来的排版，包含逗号、句点与针数标注。
...
- **每圈开头的锁针，只有在织图说它算针数时才计入**：`ch 3 (counts as dc)` 算它所代替的那 1 针，`ch 1 (does not count as a st)` 算 0 针；没有加注时，若该圈接合到这串锁针的顶端就算 1 针，接合到别处就算 0 针。写成步骤的 `mr` 与结尾接合的 `sl st` 一律算 0——它们会被画出来，但属于指示而不是织物的针目。
- 写在**一行中间**的锁针是真正的针目，每一针算 1，即使下一圈钩入的是这串锁针形成的空间而不是锁针本身。`R2: ch 1 (does not count as a st), sc in same st, ch 1, [sc in next dc, ch 1] x23, sl st to first sc` 算 48 针：24 短针加 24 锁针。
- 狗牙针（picot）、移动用的 `sl st into next ch-1 sp`、`turn`、`skip` 一律算 0。
- 锁针空间本身不是一针：它只以形成它的那些锁针计数。
...
- `tool: on` —— 在织图旁显示完整的交互式检查清单：逐行列表、进度条，以及按权重前进的每行针数计数器。每完成一个书写单位点击一次；按钮会增加该单位的权重（`inc` 是 `+2`，V 针／贝壳针会一次增加完整权重）。织图也会实时标出当前的行／针。
...
10. 在原文明确换线的地方加上 `color <name>` 步骤，使用原文写的确切颜色词或色码；原文没提到颜色的地方就不要加。
