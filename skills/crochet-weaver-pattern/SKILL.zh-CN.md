...
| `style` | `radial` \| `japanese` \| `continuous` | 插件设置值 | 环织织图的绘制样式。`japanese` 为日本钩织书风格：一条连续螺旋线绕过每一圈、普通针跟随记录的父针、加针以父针为中线画成平衡 **V**、减针画成 **∧**、标注圈数，并把 `in MR` 中心印成 `わ`。`in ch ring` 中心仍绘制锁针符号环，不使用 `ち`。`continuous` 使用同一套来源排版，但每一针都画出自己的符号并连线到下方针目。使用自动圈距时，拥挤的圈可以扩大；明确指定 `spacing` 时，每两圈都严格使用同一个精确距离。只要设置尺寸的针目能排开就保留父针角度；继承位置如果发生碰撞，系统会用确定性的最小位移恢复净空，不再套用每圈各自的局部缩放。修正角度会沿记录的来源关系向内协调，让外圈针目仍对准画面上的父针或 V 端点，并让每个 V 继续以真实来源为中线。所有圈都会保留由用户设置解析出的针目尺寸。这能防止重叠与隐形针目，也不依赖特定圈数或针数。刻意跳针不会触发无关的平均分布，圈号／分隔线组合也只能在测得的接缝余量内移动。每个圈号会从测得的安全位置沿连续无碰撞路径前进，并在第一个碰撞处停下，因此会留在分隔线的起圈侧并完整位于白色接合点左侧；从任何圈数开始的截取织图也使用同一规则。只影响 `type: round`。 |
