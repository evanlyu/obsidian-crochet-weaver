...
| `style` | `radial` \| `japanese` \| `continuous` | プラグイン設定 | 輪編みチャートの描画スタイル。`japanese` は日本の編み物本風：連続したらせん線が各段を巡り、普通の目は記録された親に従い、増し目は親を中心に均整の取れた **V**、減らし目は **∧** として描かれ、段数が表示され、`in MR` の中心は `わ` と表示されます。`in ch ring` は `ち` と省略せず、鎖編み記号の輪を描きます。`continuous` は同じ親由来の配置で各針目の記号を描き、下の編み入れ先と線でつなぎます。自動間隔では混み合う段を広げられます。`spacing` を明示した場合はすべての段間で同じ厳密な距離を使います。設定サイズの記号が収まる限り親の角度を保ち、継承位置が重なる場合は決定的な最小移動で余白を戻し、段ごとの局所的な縮小は行いません。補正した角度は記録された目の関係に沿って内側へ整合されるため、外段の目は画面上の親の目または V の端点にそろい、各 V も実際の親を中心に保ちます。すべての段はユーザー設定から解決された針目サイズを保ちます。これにより、特定の段数や目数に依存せず、重なりと見えない針目を防ぎます。意図的な飛ばし目で無関係な均等配置へ切り替えず、段数と区切り線は実測した継ぎ目の余裕の中だけで動きます。各段数表示は実測した安全位置から衝突のない連続経路だけを進み、最初の衝突で止まるため、区切り線の開始側に残り、白い接合点より完全に左側へ置かれます。どの段から始まる部分チャートにも同じ規則を使います。`type: round` にのみ影響します。 |
