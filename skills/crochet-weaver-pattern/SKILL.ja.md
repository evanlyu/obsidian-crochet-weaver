...
段が次の段に差し出す「場所」は、その段の針目（`st`、`sc`、`dc`、`picot` など）**と、針目のあいだの鎖の空間**です。2つの針目のあいだに書かれた鎖のひと続きは、1つの `ch-N sp` になります。`next <場所>` は途中にあるものを自動的に通り越すので、とばす目を書き出す必要はありません──原文も書いていません。
