...
| `id` | 1～80 文字の ASCII 英字、数字、`_`、`-` | ブロックの内容から自動生成 | `crochet-tool`／`tool: on` パネルを併用する場合は必ず設定してください。パターン文を後で編集しても進捗が保持されます。 |
...
| `style` | `radial` \| `japanese` \| `continuous` | プラグイン設定 | 輪編みチャートの描画スタイル。`japanese` は日本の編み物本風：連続したらせん線が各段を巡り、普通の目は記録された親に従い、増し目は親を中心に均整の取れた **V**、減らし目は **∧** として描かれ、段数が表示され、`in MR` の中心は `わ` と表示されます。`in ch ring` は `ち` と省略せず、鎖編み記号の輪を描きます。`continuous` は同じ親由来の配置で各針目の記号を描き、下の編み入れ先と線でつなぎます。自動間隔では混み合う段を広げられます。`spacing` を明示した場合はすべての段間で同じ厳密な距離を使い、チャート全体で 1 段目の半径を上げて設定サイズの記号を収められます。継承位置が重なる場合は決定的な最小移動で余白を戻し、段ごとの局所的な縮小は行いません。補正角度を内側へ整合するのは、前段の各ソースをちょうど 1 回ずつ覆う場合だけです。一部または重複ソースでは回写を止め、以前の目と V の形を保ちます。意図的な飛ばし目で無関係な均等配置へ切り替えません。段数と区切り線は実測した継ぎ目の余裕の中を衝突のない連続経路だけ進み、最初の衝突で止まるため、区切り線の開始側に残り、白い接合点より完全に左側へ置かれます。部分チャートにも同じ規則を使います。`type: round` にのみ影響します。 |
| `lace` | 真偽値（上記と同様） | `off` | 編み物本のレース表示：段のガイドと段数を隠し、記号を大きくし、鎖の連なりを透かし模様側へ立ち上げます。 |
| `sector` | `on` または 0～360 の数値 | 全体表示 | 輪編みチャートの 1 つの扇形だけを描きます。`on` は 90 度です。 |
| `wholeRounds` | 正の整数 | なし | `sector` を適用する前に、中心からこの段数までを全周で残します。 |
| `grid` | 真偽値（上記と同様） | プラグイン設定 | 実際のチャートに揃えた背景参考グリッドを描きます。 |
| `rounds` / `rows` / `columns` | 正の整数 | チャートの実範囲 | 背景参考グリッドを実際のパターンより外へ延長します。縮小はしません。 |
...
| `readable` | 真偽値（上記と同様） | プラグイン設定 | ツール／テキストパネルに略記ではなく、翻訳済みの完全な針目名を表示します。 |
| `position` | `right` \| `left` \| `below` | プラグイン設定 | 埋め込んだ `tool`／`text` パネルを編み図に対してどこに配置するか。 |

無効な値は対応する全体設定に静かにフォールバックし、`sector` のように全体の既定値がない項目では無視されます。エラーにはならないので、確信が持てない値は自分でよく確認してください。

グラフ関係で配置する輪編みチャートで `spacing` を明示した場合、すべての段間は厳密に同じ距離を保ちます。後の段で記号の数や幅が急増するときは、先にチャート全体を実測し、収まるために必要な分だけ 1 段目の絶対半径を大きくします。不可能な小さい円周を保つために、目を重ねたり、隠したり、段ごとに縮小したりはしません。投影後の角度を内側へ整合するのは、直前の段の各ソースをちょうど 1 回ずつ完全に覆う関係だけです。一部の位置だけを意図的に使う場合や同じソースを繰り返し使う場合はそこで整合を止め、以前の目の位置と均整の取れた V を保ちます。
...
段の最初の立ち上がりの鎖編みは**対応しています**。原文の通常または注記付きの `ch` ステップをそのまま残してください（`R2: ch, [2 sc, inc] rep, slst`）。継ぎ目に描かれ、`ch 3 (counts as dc)` は代わりの 1 目、`ch 1 (does not count as a st)` は 0 目です。注記なしの立ち上がりは、その段が最後に鎖の頭へ接合するときだけ 1 目として数えます。段を閉じる `sl st` と、ステップとして書いた `mr`（`R1: mr, ch, sc6, slst`）は常に 0 目です。
...
それ自体には幅がありません（目数には数えません）。このステップ以降のすべての針目──この段の残りと、それ以降のすべての段──にこの色が適用され、目数は通常どおり数えられます。次の `color` ステップで変更されるまで有効で、「色指定なしに戻す」というトークンはありません。針目の記号そのものは常に編み図本来のテーマカラーのままです──`black`／`white` のような文字どおりの色は毛糸の色を示すためのラベルであり、実際にその色で描画されるわけではありません（ダークテーマやライトテーマで見えにくくなったり、進捗ツール自身の「現在位置」ハイライトと競合したりするためです）。その代わり、新しい色に変わった最初の針目の横にその色の小さな中抜きの円を描き、針目の記号自体は隠さずに切り替え位置がひと目で分かるようにしています。進捗ツールとパターン文字パネルにも、切り替わる位置に「`<color>` に変更」とそのまま表示されます。

原文に書かれている色の単語やカラーコードをそのまま使ってください（`white`、`black`、`#ff8800` など）──原文にない色を考え出したり置き換えたりせず、原文が色をまったく指定していないパーツには `color` ステップを追加しないでください。

### レース：どこに編み入れるかを書く

レースの編み図は、前段を数えながら進むのではなく、1目ずつ「どこに編み入れるか」を書きます。原文どおり、`in` と場所を書いてください。

```
sc in next dc                          → 前段の次の長編みに
5 dc in next ch-2 sp                   → 長編み5目を、次の鎖の空間にまとめて
dc in same st                          → 直前のステップと同じ場所に
sc in center dc of next 7-dc shell     → 次の7長編みシェルの中央の目に
sl st into next ch-1 sp                → 引き抜きでその空間まで移動（まだ何も編み入れない）
skip 2                                 → 2か所を意図的にとばす
```

段が次の段に差し出す「場所」は、その段の針目（`st`、`sc`、`dc`、`picot` など）**と、針目のあいだの鎖の空間**です。2つの針目のあいだに書かれた鎖のひと続きは、1つの `ch-N sp` になります。`next <place>` は途中にあるものを自動的に通り越すので、とばす目を書き出す必要はありません──原文も書いていません。

**シェルと V ステッチ。** 「数量＋場所」は、1か所に編み入れる1つのモチーフで、その場所から開く扇として描かれます。

```
5 dc in next ch-2 sp                   → 5長編みのシェル1つ
V2 in next sc                          → (dc, ch 2, dc) を同じ目に。V3 は鎖3の版
(dc, ch 2, dc) in next sc              → 同じものを展開して書いた形
```

V の内側の鎖はそれ自体が空間なので、次の段は `in next ch-2 sp` と書けます。奇数のシェルには中央の目があり、あとの段から `in center dc of next 5-dc shell` と指定できます。

**立ち上がりの鎖、つなぎ、返し。**

```
ch 3 (counts as dc)                    → この鎖が段の最初の長編みの代わりになる
ch 1 (does not count as a st)          → 高さだけ
sl st to top of beginning ch-3         → 立ち上がりの鎖の頭に引き抜いてとじる
sl st to first sc
sl st to join
turn                                   → 段の先頭に書く：この段は逆向きに編む
```

注記の**ない**立ち上がりの鎖は、その段自身のつなぎ先で決まります。立ち上がりの鎖の頭にとじていれば1目の代わり、それ以外にとじていれば目数に入りません。ですから `ch 3` とだけ書いて `top of beginning ch-3` にとじる原文は、注記を足す必要はありません。そのまま書いてください。

**前の段の繰り返しとして書かれた段。** ダッシュはどちらの形でもよく、チャートを描く前に実際の段へ展開されます。
...
**本のように印刷する。** レースの編み図の frontmatter に `lace: on` を加えます。段を囲む線を引かず、段数も書かず、透かしに映えるよう記号を大きめに描きます。変わるのはパターンの周りに何を描くかだけで、パターンそのものは変わりません──編み物本のような見た目を求められたら付け、段を囲んで番号を振ってほしいときは付けません。

**円全体ではなく一区画だけ。** `sector: 90`（または `sector: on`）は、`wholeRounds: 4`（最初の数段は丸ごと）と併せて、輪編みチャートの扇形の一切れだけを描きます。同じモチーフが12個並ぶ段を、本もそう印刷します。円盤ではなく扇として見せたいとき、あるいは一周では大きすぎて見づらいときに付け、繰り返しをすべて見せたいときは付けません。

**1つの段を複数行に書けます。** 字下げされた行はその上の段の続きで、空行で段が終わります。長い段は、カンマ・句点・目数注記も含めて、原文の体裁のまま書けます。
...
- **段の最初の鎖編みは、編み図がそう言っているときだけ数えます**。`ch 3 (counts as dc)` は代わりになる 1 目として数え、`ch 1 (does not count as a st)` は 0 目。注記がない場合は、その段が立ち上がりの鎖の頭にとじていれば 1 目、それ以外にとじていれば 0 目です。ステップとして書いた `mr` と段を閉じる `sl st` は常に 0 目──描画はされますが、編み地の目ではなく指示だからです。
- **段の途中**にある鎖編みは本物の針目で、1 つずつ 1 目に数えます。次の段が編み入れるのは、その鎖のひと続きが作る空間のほうですが、数えるのは鎖です。`R2: ch 1 (does not count as a st), sc in same st, ch 1, [sc in next dc, ch 1] x23, sl st to first sc` は 48 目：細編み 24 目と鎖 24 目です。
- ピコット、移動のための `sl st into next ch-1 sp`、`turn`、`skip` はいずれも 0 目です。
- 鎖の空間そのものは 1 目ではありません。数えるのは、それを作った鎖だけです。
...
- `tool: on` — 編み図の横に完全なインタラクティブチェックリストを表示します：段ごとのリスト、進捗バー、重みに応じて進む各段の目数カウンターです。書かれた単位を 1 つ編むごとに 1 回押し、ボタンはその単位の重みを加算します（`inc` は `+2`、V ステッチ／シェルは単位全体の重みを一度に加算）。編み図も現在の段／目をリアルタイムでハイライトします。
...
10. 原文が明確に糸の色を変えている箇所には、原文どおりの色の単語やカラーコードで `color <name>` ステップを追加した。原文が色を指定していない箇所には追加していない。
