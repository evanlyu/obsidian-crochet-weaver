...
- **Turns apply to the current round.** A `turn` at the start of R4 and later makes that round read the round below it backwards and toggles its RS/WS state. Which way round the chart it is then drawn is not a separate choice: a stitch is drawn where the place it is worked into puts it, so the round runs the round below's way about the chart, reversed only where it reads that round backwards.
...
- **A chart can say what kind of chart it is.** `lace: on` prints it the way a pattern book prints lace — no lines around the rounds, no round numbers, symbols drawn larger against the openwork — and `sector: 90` draws one wedge of it rather than the whole circle. Neither changes what the pattern is or what the chart is made of; they change what is drawn around it.
...
- **Not chart syntax**: button loop and sewing instructions, gauge notes, and the reference page's mixed circular/strip presentation. (Drawing one wedge of a chart *is* supported, as a drawing option — see `sector` above — but it is a way of printing the chart, not a thing the pattern says.)
