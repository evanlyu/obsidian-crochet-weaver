...
- 每一針都會保留真實來源對齊，圈號與換圈分隔則只在量得的縫隙內避開針目。
...
底層上，每一針都由「上一圈所鉤入的針目」決定偏好位置並記錄下來：普通針精準保留唯一父針的角度，加針的兩針以共用父針為中線對稱展開，減針則位在所有來源針之間。使用自動圈距（未指定 `spacing`）時，擁擠的圈會擴到能容納這些來源角度的最小半徑；明確指定 `spacing` 時，每兩圈的距離都嚴格固定為該值。排版前會先量完整張圖；若最擁擠的一圈需要更長圓周，系統只把第 1 圈的絕對半徑提高到足以容納所有設定尺寸的符號與接縫，之後每圈仍精準相差設定圈距。只要針目排得開就完整保留父針角度；若繼承目標碰撞，系統會用確定性的最小位移恢復順序與淨空，不套用每圈各自的局部縮放。修正角度只有在前一圈每個來源都恰好被覆蓋一次時才往內協調；刻意選用部分來源或重複使用來源會形成邊界，讓目前圈自行排開，不會拉壞較早圈的 V 或針目。所有圈都保留使用者設定所解析出的針目尺寸。這些規則只看實測幾何，不依賴特定圈數、針法、針數或文字寫法，也不會刪掉任何針目。

`style: continuous` 使用同一套排版，但把對應關係直接畫出來而不是用書上的符號表示：每一針都保留自己的符號（**包含加針產生的兩針**），再從它們畫線連到上一圈所鉤入的針目。適合用來檢查織圖，或在還不熟悉書上符號時閱讀。

兩種樣式都不會把普通的一對一針目任意分組或擠壓：每一針都優先跟著它所鉤入的那一針。寫成重複的一圈——例如 `[2 sc, inc] x 6`——之所以能看出六個扇形，是因為那六個加針跟著各自所鉤入的六針，而不是因為織圖把它們湊在一起。既沒有寫重複、也沒有加減針的一圈——例如籃子筒身的 `R9: 40 sc`——會照抄下面那一圈。自由織圖即使刻意跳過某些位置，仍以每個記錄來源作為偏好目標；若固定圈距必須投影才能維持淨空，修正會停在部分或重複來源的關係邊界，不會拖動較早圈，也不會改成與來源無關的平均分佈。

第一圈鉤入中心環的第一針固定在十二點鐘方向。圈號從其右側開始，每往外一圈以再朝十二點鐘方向半度為目標，形成微微向內收的導線而不是僵硬輻條。幾何空間允許時，縫隙會保留接合、換圈階梯、圈號與起立鎖針所需的空間；兩側 10px 淨空是最小值，指定圈距則是不可放大的精確值。來源留下的額外縫隙空間會完整保留，圈號／分隔線封包只能在量得的縫隙餘裕中移動；若會越過最後一針就提早停下，不會為了對準圈號而旋轉針目。排版會從量得的安全圈號位置沿著連續無碰撞的路徑朝目標角度移動，並在第一個碰撞邊界停下；即使另一側還有空位，圈號也會留在分隔線的起圈側，並完整位於白色接合點左側。這套規則依照實測的數字與符號尺寸運作，因此截取織圖從 R4、R16 或任何圈數開始都同樣適用。
