...
底層上，每一針都由「上一圈所鉤入的針目」決定位置並記錄下來：普通針精準保留唯一父針的角度，加針的兩針以共用父針為中線對稱展開，減針則位在所有來源針之間。使用自動圈距（未指定 `spacing`）時，擁擠的圈會擴到能容納這些來源角度的最小半徑；明確指定 `spacing` 時，每兩圈的距離都嚴格固定為該值。只要針目能以設定尺寸排開，就會完整保留父針角度；若連續數圈在不同位置加減針，使繼承的目標互相碰撞，系統會用確定性的最小位移恢復鉤織順序與淨空，不再套用每圈各自的局部縮放。修正後的角度會沿記錄的針目關係往內協調，因此外圈針目仍直接對準畫面上的父針或加針 V 端點，每個 V 也持續以真實來源為中線。所有圈的針目都會統一保留使用者設定所解析出的尺寸。這項規則只看實際幾何，不依賴特定圈數或針數，會維持平衡的 V，也不會刪掉任何針目。

`style: continuous` 使用同一套排版，但把對應關係直接畫出來而不是用書上的符號表示：每一針都保留自己的符號（**包含加針產生的兩針**），再從它們畫線連到上一圈所鉤入的針目。適合用來檢查織圖，或在還不熟悉書上符號時閱讀。

兩種樣式都不做分組、也不刻意擠壓：每一針就直接跟著它所鉤入的那一針。寫成重複的一圈——例如 `[2 sc, inc] x 6`——之所以能看出六個扇形，是因為那六個加針各自落在它們所鉤入的六針正上方，而不是因為織圖把它們湊在一起。既沒有寫重複、也沒有加減針的一圈——例如籃子筒身的 `R9: 40 sc`——會照抄下面那一圈。自由織圖即使刻意跳過某些位置，實際鉤出的每一針仍會對齊自己記錄的來源，不會整圈改成與來源無關的平均分佈。
