...
每圈開頭的起立鎖針與結尾的引拔針會畫在該圈的接縫處，但都不算進該圈的針數：上一圈鉤入的是它們之間的針目。
...
加針與減針比照鉤織書的畫法：它們是**自己那一圈的符號**，和普通針排在同一條帶子上。`inc` 是一個 **V**，尖點落在該圈內側圈線、對齊它所鉤入的那一針，兩臂張開到該圈外側圈線，各代表它產生的一針，也就是下一圈要鉤入的位置。`dec`（以及 `scNtog`）則是 **∧**：兩腳分別落在被併掉的兩針上，尖點立在其上方。沒有任何東西浮在兩圈之間，也沒有固定圖形：每個符號都依該圈的帶寬決定大小，並朝它所屬的針目傾斜。它會開到足以跨過所屬的那幾針，但不會開到失去 V 的形狀：在外圈把相隔很遠的兩針併起來時，符號仍然朝著兩者張開，而不會被拉成兩條長線。
...
兩種樣式都不做分組、也不刻意擠壓：每一針就直接跟著它所鉤入的那一針。寫成重複的一圈——例如 `[2 sc, inc] x 6`——之所以能看出六個扇形，是因為那六個加針各自落在它們所鉤入的六針正上方，而不是因為織圖把它們湊在一起。既沒有寫重複、也沒有加減針的一圈——例如籃子筒身的 `R9: 40 sc`——會完全照抄下面那一圈，因此連續的普通圈會沿著下方加減針留下的位置，往外疊成筆直的放射狀直行。
...
- **圓織圖樣式**：`standard`（針目平均分佈）、`japanese`（連續螺旋圈線、依上一圈決定針目位置、書上的 V／∧ 加減針符號、標示圈數）或 `continuous`（同一套排版，每一針都畫出並用線連到上一圈）。
