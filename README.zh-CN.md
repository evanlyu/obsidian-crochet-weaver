...
- 每一针都会保留真实来源对齐，圈号与换圈分隔只在测得的接缝空间内避开针目。
...
底层上，每一针都由「上一圈所钩入的针目」决定首选位置并记录下来：普通针精确保留唯一父针的角度，加针的两针以共用父针为中线对称展开，减针则位于所有来源针之间。使用自动圈距（未指定 `spacing`）时，拥挤的圈会扩大到能容纳这些来源角度的最小半径；明确指定 `spacing` 时，每两圈的距离都严格固定为该值。排版前会先测量整张图；如果最拥挤的一圈需要更长圆周，系统只把第 1 圈的绝对半径提高到足以容纳所有设置尺寸的符号与接缝，之后每圈仍精确相差设置圈距。只要针目排得开就完整保留父针角度；如果继承目标碰撞，系统会用确定性的最小位移恢复顺序与净空，不套用每圈各自的局部缩放。修正角度只有在前一圈每个来源都恰好被覆盖一次时才向内协调；刻意选用部分来源或重复使用来源会形成边界，让当前圈自行排开，不会拉坏较早圈的 V 或针目。所有圈都保留由用户设置解析出的针目尺寸。这些规则只看实测几何，不依赖特定圈数、针法、针数或文字写法，也不会删除任何针目。

`style: continuous` 使用同一套排版，但把对应关系直接画出来而不是用书上的符号表示：每一针都保留自己的符号（**包含加针产生的两针**），再从它们画线连到上一圈所钩入的针目。适合用来检查织图，或在还不熟悉书上符号时阅读。

两种样式都不会把普通的一对一针目任意分组或挤压：每一针都优先跟着它所钩入的那一针。写成重复的一圈——例如 `[2 sc, inc] x 6`——之所以能看出六个扇形，是因为那六个加针跟着各自所钩入的六针，而不是因为织图把它们凑在一起。既没有写重复、也没有加减针的一圈——例如篮子筒身的 `R9: 40 sc`——会照抄下面那一圈。自由织图即使刻意跳过某些位置，仍以每个记录来源作为首选目标；如果固定圈距必须投影才能维持净空，修正会停在部分或重复来源的关系边界，不会拖动较早圈，也不会改成与来源无关的平均分布。

第一圈钩入中心环的第一针固定在十二点方向。圈号从它右侧开始，每向外一圈以再朝十二点方向半度为目标，形成略向内收的引导线而不是僵硬的辐条。几何空间允许时，接缝会保留接合、换圈台阶、圈号与起立锁针所需的空间；两侧 10px 净空是最小值，指定圈距则是不可放大的精确值。来源留下的额外接缝空间会完整保留，圈号／分隔线组合只能在测得的接缝余量中移动；如果会越过最后一针就提前停下，不会为了对准圈号而旋转针目。排版会从测得的安全圈号位置沿着连续无碰撞的路径向目标角度移动，并在第一个碰撞边界停下；即使另一侧仍有空位，圈号也会留在分隔线的起圈侧，并完整位于白色接合点左侧。这套规则按实测的数字与符号尺寸运算，因此截取织图从 R4、R16 或任何圈数开始都同样适用。
