...
`style: book` 会把环织织图切换成日本钩织书的风格：用一条连续螺旋线绕过每一圈（实际上圈织就是一条连续螺旋，而不是一圈圈独立的封闭圆），在每圈起点缝线处往外跨到下一圈；每一针对齐上一圈所钩入的针目（加针从母针展开、减针收拢），加针／减针符号本身会拉宽成书上那种 V／∧ 形状连到对应的针目，并在缝线处用红色标注圈数。默认的 `style: standard` 保持原本的平均分布画法；全局的「圆织图样式」设置可以改变所有织图的默认值。
...
- **圆织图样式**：`standard`（针目平均分布）或 `book`（连续螺旋圈线、针目对齐上一圈、拉宽的加减针符号、标注圈数）。
