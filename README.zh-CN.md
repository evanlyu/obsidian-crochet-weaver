...
- **按原文绘制蕾丝花样**：可以直接写出钩入位置（`5 dc in next ch-2 sp`、`sc in center dc of next 7-dc shell`）；贝壳针会从来源空间展开，锁针沿实际弧线排列，V 针、狗牙针、接合、翻面与来源圈重复都会按原文解析。
- 圈号与换圈接缝会避开针目，同时保留远离接缝处的放射状针目对齐。
...
- 所有进度都存储在本机的插件数据文件中。
- 完整支持八种界面语言：英文、繁体中文、简体中文、日文、韩文、德文、法文、西班牙文。
...
想让进度在修改织图文字后依然保留，请设置明确的 `id`。它必须是 1–80 个 ASCII 英文字母、数字、`_` 或 `-`；省略或无效的 `id` 会回退到由代码块内容生成的本机哈希值，因此修改代码块内容可能会重置进度。
...
有全局设置的选项默认会使用全局值；frontmatter 中的有效值只覆盖当前织图，无效值则回退到对应全局值。`sector` 这类只属于单张织图的显示选项如果无效会被忽略。蕾丝、扇形、背景网格与面板选项会在下方各节说明。
...
每圈开头的起立锁针与结尾接合的引拔针都会画在接缝处；接合引拔针始终算 0。起立锁针如果写成 `ch 3 (counts as dc)`，或没有加注但最后接合到它的顶端，就算它所代替的 1 针；`ch 1 (does not count as a st)` 或接合到别处的起立锁针则算 0。
...
加针与减针比照钩织书的画法：它们是**自己那一圈的符号**，和普通针排在同一条带子上。`inc` 是一个 **V**，尖点落在该圈内侧圈线的来源针位置，两臂直接终止在它实际产生的两个子针位置。`dec`（以及 `scNtog`）则是 **∧**：两脚朝被并掉的针目倾斜，尖点立在其上方；来源异常宽时会把减针符号收紧，保持可读性。没有任何东西浮在两圈之间，每个端点都持续表达真实的针目来源关系。
...
两种样式都不做分组、也不刻意挤压：每一针就直接跟着它所钩入的那一针。写成重复的一圈——例如 `[2 sc, inc] x 6`——之所以能看出六个扇形，是因为那六个加针各自落在它们所钩入的六针正上方，而不是因为织图把它们凑在一起。既没有写重复、也没有加减针的一圈——例如篮子筒身的 `R9: 40 sc`——会照抄下面那一圈；足够大的普通圈如果需要修正圈号接缝，修正只会在最靠近接缝的四分之一圈内逐渐消失，对侧仍保持精确的放射状直行。

第一圈钩入中心环的第一针固定在十二点方向。圈号从它右侧开始，每向外一圈再朝十二点方向倾斜半度，形成略向内收的引导线而不是僵硬的辐条。接缝始终会保留接合、换圈台阶、圈号与起立锁针所需的空间；大的普通圈最多可保留 10px 的额外继承空间，避免为了关闭接缝而不必要地把针目拉离来源。
...
- `tool: on`——嵌入完整的交互式进度工具（行清单、进度条、按权重增减的每行针数计数器，以及上一圈／完成本圈／重置整体进度控制）。
...
- **针数计数器**——针对当前这一行，每完成一个书写单位点击一次加号；按钮会按该单位的针数权重显示并增加，例如 `inc` 是 `+2`，V 针与贝壳针也会一次增加整个单位的权重。计数达到该行总针数时会自动完成该行并将计数器归零。减号会按上一个单位的权重修正误点，重置按钮只清空当前行的计数，不影响行进度。点击上一圈、完成本圈、重置整体进度或其他行，都会把新的当前行计数器归零。
...
- **织图文字样式**：选择原始简码或完整翻译的易读针法名称，可用 `readable: on/off` 单独覆盖。
- **面板位置**：嵌入的工具或文字面板的默认位置（右／左／下方），可用 `position:` 单独覆盖。
- **显示背景参考网格**：让每张 `crochet` 织图默认显示圈数／行列参考网格，可用 `grid: on/off` 单独覆盖。
- **圆织图样式**：`radial`（针目平均分布）、`japanese`（连续螺旋圈线、依上一圈决定针目位置、书上的 V／∧ 加减针符号、标注圈数）或 `continuous`（同一套排版，每一针都画出并用线连到上一圈）。
