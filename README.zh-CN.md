...
底层上，每一针都由「上一圈所钩入的针目」决定位置并记录下来：普通针精确保留唯一父针的角度，加针的两针以共用父针为中线对称展开，减针则位于所有来源针之间。使用自动圈距（未指定 `spacing`）时，拥挤的圈会扩大到能容纳这些来源角度的最小半径；明确指定 `spacing` 时，每两圈的距离都严格固定为该值。只要针目能以设置尺寸排开，就会完整保留父针角度；如果连续数圈在不同位置加减针，使继承的目标互相碰撞，系统会用确定性的最小位移恢复钩织顺序与净空，不再套用每圈各自的局部缩放。修正后的角度会沿记录的针目关系向内协调，因此外圈针目仍直接对准画面上的父针或加针 V 端点，每个 V 也继续以真实来源为中线。所有圈的针目都会统一保留由用户设置解析出的尺寸。这项规则只看实际几何，不依赖特定圈数或针数，会维持平衡的 V，也不会删除任何针目。

`style: continuous` 使用同一套排版，但把对应关系直接画出来而不是用书上的符号表示：每一针都保留自己的符号（**包含加针产生的两针**），再从它们画线连到上一圈所钩入的针目。适合用来检查织图，或在还不熟悉书上符号时阅读。

两种样式都不做分组、也不刻意挤压：每一针就直接跟着它所钩入的那一针。写成重复的一圈——例如 `[2 sc, inc] x 6`——之所以能看出六个扇形，是因为那六个加针各自落在它们所钩入的六针正上方，而不是因为织图把它们凑在一起。既没有写重复、也没有加减针的一圈——例如篮子筒身的 `R9: 40 sc`——会照抄下面那一圈。自由织图即使刻意跳过某些位置，实际钩出的每一针仍会对齐自己记录的来源，不会整圈改成与来源无关的平均分布。
